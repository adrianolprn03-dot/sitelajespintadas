...
export default function EditarQuadroServidorPage({ params }: { params: { id: string } }) {
    ...
    useEffect(() => {
        const fetchQuadro = async () => {
            try {
                const res = await fetch(`/api/quadro-servidores/${params.id}`);
                if (res.ok) {
                    const data = await res.json();
                    setFormData({
                        cargo: data.cargo,
                        vinculo: data.vinculo,
                        leiCriacao: data.leiCriacao || "",
                        vagasLei: data.vagasLei,
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-6">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-2xl font-bold text-gray-800">Editar Cargo no Quadro</h1>
                    <p className="text-gray-500">Atualize informações do Quadro de Pessoal.</p>
                </div>
            </div>
