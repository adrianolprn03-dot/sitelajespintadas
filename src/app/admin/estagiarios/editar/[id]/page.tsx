...
import { FaArrowLeft, FaSave, FaUserGraduate, FaSpinner } from "react-icons/fa";

export default function EditarEstagiarioPage({ params }: { params: { id: string } }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await fetch(`/api/admin/estagiarios/${params.id}`);
                ...
                if (data) {
                    setForm({
                        ...
                        valorBolsa: data.valorBolsa.toString(),
                        status: data.status
                    });
                }
            } catch {
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-6">
            <div className="flex items-center justify-between">
                <Link href="/admin/estagiarios" className="text-gray-500 hover:text-gray-700 flex items-center gap-2 transition-colors">
                    <FaArrowLeft /> Voltar
                </Link>
            </div>
            ...
            <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
                <div className="p-8 border-b border-gray-100 bg-gray-50/50">
                    <h1 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
                        <FaUserGraduate className="text-primary-500" /> Editar Estagiário
                    </h1>
                    <p className="text-gray-500 text-sm">Atualize as informações do estagiário.</p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="p-8 space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            <label className="block text-sm font-bold text-gray-700 uppercase tracking-widest mb-2">Instituição de Ensino *</label>
                            <input
                                type="text"
                                className="input-field"
                                value={form.instituicaoEnsino}
                                onChange={(e) => setForm({ ...form, instituicaoEnsino: e.target.value })}
                                required
                            />
                        </div>
                        ...
                        <div>
                            <label className="block text-sm font-bold text-gray-700 uppercase tracking-widest mb-2">Valor da Bolsa (R$) *</label>
                            <input
                                type="number"
                                step="0.01"
                                className="input-field"
                                value={form.valorBolsa}
                                onChange={(e) => setForm({ ...form, valorBolsa: e.target.value })}
                                required
                            />
                        </div>
