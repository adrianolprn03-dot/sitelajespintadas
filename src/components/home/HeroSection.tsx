...
export default function HeroSection({ linksExternos = [] }: HeroSectionProps) {
    ...
    return (
        <section className="relative w-full min-h-[88vh] md:min-h-[92vh] flex items-center justify-center overflow-hidden">
            ...
            <div className="absolute inset-0 z-0 overflow-hidden">
                {!videoError ? (
                    <video
                        ...
                        className="w-full h-full object-cover brightness-[1.06] contrast-[1.03] scale-105 transition-all duration-700"
                    />
                ) : (
                    <img
                ...
                {/* Overlay gradiente suave leve para manter a visibilidade total do vídeo */}
            ...
            {/* Conteúdo Principal */}
            <div className="relative z-10 w-full max-w-[1240px] mx-auto px-4 sm:px-6 flex flex-col items-center text-center pt-32 md:pt-40 pb-16 md:pb-24 animate-fade-in-up">
                ...
                <p className="text-base sm:text-xl text-white/90 max-w-xl mb-10 font-bold drop-shadow-[0_2px_10px_rgba(0,0,0,0.9)] leading-relaxed tracking-wide">
                    Cuidando da nossa gente e Construindo o nosso futuro.
                </p>

                {/* Grid dos 4 Cards Ultra-Transparentes e Compactos */}
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 w-full max-w-4xl">
                    {cards.map((item, idx) => {
                        ...
                        return (
                            <Link
                                ...
                                className="group flex flex-col items-center text-center p-4 sm:p-5 rounded-[1.6rem] bg-white/10 hover:bg-white/20 backdrop-blur-md border border-white/25 hover:border-amber-300/60 shadow-xl transition-all duration-300 hover:-translate-y-1 relative"
                            >
                                <div className="w-11 h-11 rounded-xl bg-white/15 border border-white/20 flex items-center justify-center text-white mb-3 group-hover:scale-110 group-hover:bg-amber-400 group-hover:text-slate-950 transition-all duration-300 shadow-sm">
                                    <Icon className="text-xl" />
                                </div>

                                <h3 className="font-bold text-white text-base sm:text-lg mb-1.5 group-hover:text-amber-300 transition-colors leading-snug">
                                    {item.label}
                                </h3>

                                <p className="text-white/80 text-[11px] sm:text-xs font-normal leading-relaxed">
                                    {item.desc}
                                </p>
